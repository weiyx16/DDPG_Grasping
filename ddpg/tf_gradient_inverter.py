...
class gradient_inverter:
    def __init__(self, action_bounds):
        ...
        self.pdiff_max = tf.div(self.pmax - self.action_input, self.prange) # Normalization to prange from [0,1]
        ...
        self.grad_inverter = tf.where(tf.greater(self.act_grad, self.zeros_act_grad_filter),
            tf.multiply(self.act_grad, self.pdiff_max), tf.multiply(self.act_grad, self.pdiff_min))        
