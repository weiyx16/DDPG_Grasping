...
def linear(input_, output_size, input_other = None, stddev=0.02, bias_start=0.0, activation_fn=None, name='linear'):
    """
    ...
    if input_other == None:
        ...
        # use for combine action and state network
        shape = input_.get_shape().as_list()
        shape_other = input_other.get_shape().as_list()
        ...
        with tf.variable_scope(name):
            ...
            b = tf.get_variable('bias', [output_size], tf.float32,
                    initializer=tf.constant_initializer(bias_start))
            w_other = tf.get_variable('Matrix', [shape_other[1], output_size], tf.float32,
                    initializer=tf.random_normal_initializer(stddev=stddev))

            out = tf.nn.bias_add(tf.matmul(input_, w) + tf.matmul(input_other, w_other), b)

            if activation_fn != None:
                out = activation_fn(out)

            return out, w, w_other, b 